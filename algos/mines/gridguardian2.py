        ...
        def get_spot(a, b):
            ...
            for val in [abs((tup[0] - tup[1]) + difference(tup[0], tup[1])) for tup in zip(a, b)]:
                if board[val] == 0 and n < self.max_tiles:
                    new_val = min(abs(val - len(a)),24)
                    board[new_val] = 1
                    n += 1

        for v in range(len(self.history) - 1):
            if n < self.max_tiles:
                get_spot(self.history[v]['mineLocations'], self.history[v+1]['mineLocations'])
            else:
                break
...
algo = GridGuardian2(new_history, safe_spots)
